...
public sealed class SqliteBookRepository(string connectionString) : IBookRepository
{
    public Task<IReadOnlyList<Book>> ListAsync() =>
        Query(SqlScripts.Load("book_select.sql"));

    public async Task<Book?> GetAsync(int id)
    {
        var books = await Query(SqlScripts.Load("book_select_by_id.sql"), cmd =>
    ...
    {
        var books = await Query(SqlScripts.Load("book_insert.sql"), cmd => AddBookParams(cmd, book));
        return books[0];
    }

    public async Task<Book?> SaveAsync(Book book)
    {
        var books = await Query(SqlScripts.Load("book_update.sql"), cmd =>
        {
    ...
    public async Task RemoveAsync(int id)
    {
        ...
        cmd.CommandText = SqlScripts.Load("book_delete.sql");
        cmd.Parameters.AddWithValue("@id", id);
