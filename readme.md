...
Пачка изменений, которая либо вся сохраняется (`COMMIT`), либо вся отменяется (`ROLLBACK`) (атомарность). Пока не закоммитили — для других её как будто нет (в обычном `READ COMMITTED`).
