...
Сделки между компаниями. Не с человеком с улицы (B2C) и не с бюджетом (B2G).

---

### 4.3.5 Привести пример бизнеса процесса, нарисовать схему бизнес-процесса.

Найм программиста через тестовое: отклик, выходные с JOIN и 44-ФЗ, оффер или «мы подумаем».

```mermaid
flowchart TD
    start([Вакансия на HH]) --> apply[Отклик]
    apply --> hr{HR дозвонился?}
    hr -->|нет| ghost[Тишина]
    hr -->|да| task[Тестовое на выходные]
    task --> sql[JOIN и XML]
    sql --> poles[Два столба и провод]
    poles --> fz[Гуглить 44-ФЗ]
    fz --> draw[Нарисовать бизнес-процесс]
    draw --> send[Сдать README]
    send --> review{Зашло?}
    review -->|ещё одну схему| draw
    review -->|оффер| job([Выход в понедельник])
    review -->|подумаем| ghost
```
