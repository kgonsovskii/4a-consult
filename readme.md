...
Подсказки оптимизатору: как выполнить запрос, если его план вас не устраивает — какой индекс взять, какой join, читать ли с блокировкой. В SQL Server это `WITH (NOLOCK)`, `OPTION (RECOMPILE)` и т.п. В PostgreSQL из коробки почти нет, только расширение вроде `pg_hint_plan`.

---

### 1.5 Какие виды блокировок существуют?

По смыслу: **shared** (чтение, друг другу не мешают) и **exclusive** (запись, остальных не пускает). Ещё intent — «я собираюсь блокировать строки внутри». По месту: строка, страница, таблица.
...
`DELETE` снимает строки, можно `WHERE`, срабатывают триггеры, каждая строка пишется в лог. `TRUNCATE` сразу опустошает таблицу: быстрее, без `WHERE` и без delete-триггеров, обычно сбрасывает identity. В транзакции оба можно откатить.
